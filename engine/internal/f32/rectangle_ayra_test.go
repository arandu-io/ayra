...
func TestDxAndDyAreTheTwoHalvesOfSize(t *testing.T) {
	...
	backwards := Rectangle{Point{X: 4, Y: 12}, Point{X: 1, Y: 2}}
...
func TestStringNamesBothCorners(t *testing.T) {
	if got, want := Rect(1, 2, 3.5, 4).String(), "(1,2)-(3.5,4)"; got != want {
...
func TestEmptyIsTrueWhenAnEdgeHasNoLength(t *testing.T) {
	for _, c := range []struct {
		...
		{"area", Rect(0, 0, 1, 1), false},
		{"zero value", Rectangle{}, true},
		{"no width", Rect(1, 0, 1, 1), true},
		{"no height", Rect(0, 1, 1, 1), true},
		{"corners the wrong way round", Rectangle{Point{X: 1, Y: 1}, Point{X: 0, Y: 0}}, true},
	} {
...
func TestIntersectKeepsOnlyTheOverlap(t *testing.T) {
	r, s := Rect(0, 0, 10, 10), Rect(5, 4, 20, 20)
	if got, want := r.Intersect(s), Rect(5, 4, 10, 10); got != want {
		t.Errorf("Intersect = %v, want %v", got, want)
	}
	if got, want := s.Intersect(r), Rect(5, 4, 10, 10); got != want {
...
func TestIntersectWithoutOverlapIsTheZeroRectangle(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{"disjoint", Rect(0, 0, 1, 1), Rect(2, 2, 3, 3)},
		{"touching along an edge", Rect(0, 0, 1, 1), Rect(1, 0, 2, 1)},
		{"one of them empty", Rect(0, 0, 1, 1), Rectangle{}},
	} {
		if got := c.r.Intersect(c.s); got != (Rectangle{}) {
...
func TestUnionCoversBothRectangles(t *testing.T) {
	r, s := Rect(0, 0, 1, 1), Rect(2, 3, 4, 5)
	if got, want := r.Union(s), Rect(0, 0, 4, 5); got != want {
		t.Errorf("Union = %v, want %v", got, want)
	}
	if got, want := s.Union(r), Rect(0, 0, 4, 5); got != want {
...
func TestUnionWithAnEmptyRectangleIsTheOtherOne(t *testing.T) {
	r := Rect(1, 2, 3, 4)
	if got := r.Union(Rectangle{}); got != r {
		t.Errorf("Union with an empty rectangle = %v, want %v", got, r)
	}
	if got := (Rectangle{}).Union(r); got != r {
		t.Errorf("empty rectangle Union = %v, want %v", got, r)
	}
	if got := (Rectangle{}).Union(Rectangle{}); got != (Rectangle{}) {
...
func TestCanonOrdersCorners(t *testing.T) {
	...
	for _, r := range []Rectangle{
		...
		{Point{X: 3, Y: 2}, Point{X: 1, Y: 4}},
		{Point{X: 1, Y: 4}, Point{X: 3, Y: 2}},
...
func TestRoundGrowsToTheEnclosingIntegerRectangle(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{Rect(1.2, 2.7, 3.1, 4), image.Rect(1, 2, 4, 4)},
		{Rect(-1.2, -2.7, -0.5, -0.1), image.Rect(-2, -3, 0, 0)},
		{Rect(2, 3, 5, 7), image.Rect(2, 3, 5, 7)},
	} {
...
func TestFPtAndFRectCarryIntegerCoordinatesAcross(t *testing.T) {
	if got, want := FPt(image.Pt(3, -4)), (Point{X: 3, Y: -4}); got != want {
		t.Errorf("FPt = %v, want %v", got, want)
	}
	if got, want := FRect(image.Rect(1, 2, 3, 4)), Rect(1, 2, 3, 4); got != want {
	...
	if got, want := FRect(unordered), (Rectangle{Point{X: 3, Y: 4}, Point{X: 1, Y: 2}}); got != want {
		t.Errorf("FRect of unordered corners = %v, want %v", got, want)
	}
}

func TestPtIsTheShorthandForAPoint(t *testing.T) {
	if got, want := Pt(1.5, -2.5), (Point{X: 1.5, Y: -2.5}); got != want {
...
func TestOrderedCornersNeverGiveANegativeSize(t *testing.T) {
	...
	forEachRectangle(func(x0, y0, x1, y1 float32) {
		for _, r := range []Rectangle{
			Rect(x0, y0, x1, y1),
			Rectangle{Point{X: x0, Y: y0}, Point{X: x1, Y: y1}}.Canon(),
		} {
...
func TestRectAndCanonOrderCornersTheSameWay(t *testing.T) {
	forEachRectangle(func(x0, y0, x1, y1 float32) {
		shorthand := Rect(x0, y0, x1, y1)
		longhand := Rectangle{Point{X: x0, Y: y0}, Point{X: x1, Y: y1}}.Canon()
...
func TestTheIdentityTransformLeavesAPointWhereItIs(t *testing.T) {
	...
	transforms := []struct {
		...
		{"zero value", Affine2D{}},
		{"AffineId", AffineId()},
		{"NewAffine2D", NewAffine2D(1, 0, 0, 0, 1, 0)},
	}
...
func TestNewAffine2DKeepsTheElementsItWasGiven(t *testing.T) {
	a := NewAffine2D(2, 0.5, 3, -0.25, 4, -5)
