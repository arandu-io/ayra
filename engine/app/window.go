...
import (
	...
	"github.com/arandu-io/ayra/engine/gpu"
...
type Window struct {
	...
	decorations struct {
		...
		height        unit.Dp
		currentHeight int
		// shaper turns the title into glyphs. It is held here rather than
		// made per frame because the atlas behind it is the expensive part,
		// and the bar is redrawn as often as everything else.
		shaper *text.Shaper
		*widget.Decorations
	}
	nocontext bool
...
func (w *Window) init() {
	...
	w.decorations.shaper = shaper
	decoStyle := decorations{state: deco, shaper: shaper}
	gtx := layout.Context{
		Ops: new(op.Ops),
		// Measure in Dp.
		Metric: unit.Metric{},
...
func (w *Window) decorate(e FrameEvent, o *op.Ops) image.Point {
	...
	style := decorations{
		state:   deco,
		actions: allActions,
		title:   w.decorations.Config.Title,
		shaper:  w.decorations.shaper,
	}
